  ...
  return (
    <SelectPrimitive.Portal>
      {/* Sadece açılış animasyonu (data-open:...) var, kapanış animasyonu YOK - bkz.
          dropdown-menu.tsx ve dialog.tsx'teki aynı yorum: Radix'in Presence'ı kapanışta
          animationend event'ini beklediği için bu projede (React 19 StrictMode + mevcut
          radix-ui sürümü) o event hiç tetiklenmiyor ve içerik "kapandıktan" sonra DOM'da
          tam opaklıkta kalıyordu. Kapanış animasyon class'ları olmayınca Presence hiç
          beklemeden anında unmount ediyor - güvenilir olan bu. */}
      <SelectPrimitive.Content
        data-slot="select-content"
        className={cn(
          "relative z-50 max-h-(--radix-select-content-available-height) min-w-(--radix-select-trigger-width) origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95",
          position === "popper" &&
